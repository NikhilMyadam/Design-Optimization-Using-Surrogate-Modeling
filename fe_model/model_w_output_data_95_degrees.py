...
if os.path.isdir('Model_Data_95degrees') == True:
  print("Directory already exist")
else:
    os.mkdir('Model_Data_95degrees') #  desired directory name
    print("Created a directory - Model_Data_95degrees")

print("Current working directory: {0}".format(os.getcwd()))
os.chdir(r"<PATH_TO_PROJECT>/Model_Data_95degrees")
print("Current working directory: {0}".format(os.getcwd()))

# Start the loop for the modeling database i.e. myModel

Iterations = 17 # number of models with sill angle 95 degrees
...
def fe_model(i):
	...
	myPoint3 = (-0.05, 0.065669)
	myPoint4 = (-0.05, -0.065669)
	...
	myPoint9 = (0.05, 0.065669)
	myPoint10 = (0.05, -0.065669)
	...
	mdb.saveAs('Model_95degrees.cae')
	...
	# stacking all the arrays in a horizontal sequence (i.e. along the columns)

	HO = np.hstack((ALLAE,ALLIE,ALLKE,ETOTAL,U1_PI,U3_PI,CFN1_LS_SOI,CFN1_RS_SOI,CFN3_LS_SOI,CFN3_RS_SOI))

	# Save the history data in a single file
...
with open('model_database_95degrees_dynamic_impact_sims.csv','wb') as testfile:
